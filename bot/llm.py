...
def detect_request_type(user_text: str, is_feedback_mode: bool = False) -> list[str]:
    ...
    Args:
        ...
        is_feedback_mode: Флаг режима обратной связи (пользователь нажал 👎)
    ...
    # Проверка на feedback mode (приоритет 1)
    if is_feedback_mode:
...
def build_system_prompt(
    ...
    """
    Построить системный промпт с учетом контекста сессии и динамических блоков v1.3.4.
    Приоритет блоков: feedback -> pricing -> risks_docs -> specialist_request
    
    Args:
        context: Контекст сессии с asked_questions, collected_data и slots
        user_text: Текст запроса пользователя (для определения динамических блоков)
        is_feedback_mode: Флаг режима обратной связи (пользователь нажал 👎)
    ...
    # Добавление динамических блоков v1.3.4
    dynamic_blocks = ""
    if user_text:
    ...
    if not context:
        return base_prompt + dynamic_blocks
    ...
    # Добавление краткого резюме слотов
    slots = context.get("slots", {})
    slots_summary = build_slots_summary(slots)
    if slots_summary:
        context_addition += f"\n{slots_summary}\n"
    
    # Добавление информации о собранных данных (legacy)
...
async def generate_reply(
    ...
    # Определяем режим обратной связи из session_data
    is_feedback_mode = session_data.get("is_feedback_mode", False) if session_data else False
    
    # Build system prompt with context and dynamic blocks (v1.3.3)
